...
class StackBoxesEnv3D_multiple_obj_w_robot(StackBoxesEnv3D_multiple_obj_w_robot_inherit):
    ...
    but stabillizes calculation of IK by providing warm starts for all of the poses!
    '''
    ...
    def __init__(self, num_boxes, visualize=True, seed=None, add_connectivity=False, ensemble=False, env_grid_size=3, fully_connected=True, robot_info=False, load_path=None, num_actions=5*4, complexity_int = 4, disc_factor=None, fill_threshold=0.75):
        ...
        self.target_shapes = []
        # decide which set of objects is to be used
        if (complexity_int == 0):
    ...
    def decompose_action(self,u):
        # decompose action into placing waypoints and grasp poses which eventually lead to a change in orientation
        p_offsets = []
        grasps = []
        grasp_bias = None
        ...
        if (ac_0==0):
            ...
            p_offsets.append([0,dist,0.055-0.05+0.005])
            p_offsets.append([0,dist,0.05-0.05+0.005])
        # grasping and placing poses - also define bias which is helpful to warm start the IK solver
        grasp_bias = []
        if (ac_1 == 0):
    ...
    def step(self, u):
        # overwriting the step function is needed as we now additionally give a bias to warm start the IK and achieve
        # the desired grasping poses!
        ...
        # height offset is used to place the block correctly
        ...
        successfull = self.SimEnv.grasp_part_w_vel_check(pos, grasps[0], obj_to_be_moved, self.object_list_not_placed,invalid_idx,initial_pose=initial_pose)
        ...
        if (successfull):
            ...
            successfull = self.SimEnv.place_part_w_table_collision_check(np.asarray(pos)+offset_height, p_offsets, grasps[1], lower_obj, obj_to_be_moved,
                                                 self.object_list,self.object_list_not_placed,invalid_idx,grasp_bias=grasp_bias[1])
        if (successfull):
        ...
        # finally transfer ALL (could be multiple primitive objects) of one element into the placed elements
        ...
        if (not (successfull)):
            prior_exit = True
        # terminate if no more objects available
        finished = False
