...
class RobotBase(object):
    ...
    def __init__(self, pos, ori, phusics_client):
        """
        Arguments:
            pos: [x y z]
            ori: [r p y]
        ...
        self.physicsClient = phusics_client
        import pathlib
    ...
    def __parse_joint_info__(self):
        numJoints = p.getNumJoints(self.id,physicsClientId = self.physicsClient)
        jointInfo = namedtuple('jointInfo', 
            ['id','name','type','damping','friction','lowerLimit','upperLimit','maxForce','maxVelocity','controllable'])
        self.joints = []
        self.controllable_joints = []
        for i in range(numJoints):
            info = p.getJointInfo(self.id, i,physicsClientId = self.physicsClient)
            jointID = info[0]
            ...
            if controllable:
                ...
                p.setJointMotorControl2(self.id, jointID, p.VELOCITY_CONTROL, targetVelocity=0, force=0,physicsClientId = self.physicsClient)
    ...
    def reset_arm(self):
        ...
        for rest_pose, joint_id in zip(self.arm_rest_poses, self.arm_controllable_joints):
            p.resetJointState(self.id, joint_id, rest_pose,physicsClientId = self.physicsClient)
    ...
    def move_ee(self, action, control_method,open=True):
        ...
        if control_method == 'end':
            ...
            joint_poses = p.calculateInverseKinematics(self.id, self.eef_id, pos, orn,
                                                       ...
                                                       maxNumIterations=20,physicsClientId = self.physicsClient)
        ...
        for i, joint_id in enumerate(self.arm_controllable_joints):
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, joint_poses[i],
                                    force=self.joints[joint_id].maxForce, maxVelocity=self.joints[joint_id].maxVelocity,physicsClientId = self.physicsClient)

        if not(open):
            open_angle = 0.715 - math.asin((0.0 - 0.010) / 0.1143)  # angle calculation
            # Control the mimic gripper joint(s)
            p.setJointMotorControl2(self.id, self.mimic_parent_id, p.POSITION_CONTROL, targetPosition=open_angle,
                                    force=self.joints[self.mimic_parent_id].maxForce, maxVelocity=self.joints[self.mimic_parent_id].maxVelocity,physicsClientId = self.physicsClient)
    ...
    def get_joint_obs(self):
        ...
        for joint_id in self.controllable_joints:
            pos, vel, _, _ = p.getJointState(self.id, joint_id,physicsClientId = self.physicsClient)
            positions.append(pos)
            velocities.append(vel)
        ee_pos = p.getLinkState(self.id, self.eef_id,physicsClientId = self.physicsClient)[0]
...
class Panda(RobotBase):
    def __init_robot__(self):
        ...
        self.id = p.loadURDF(self.curr_path + '/urdf/panda.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES,physicsClientId = self.physicsClient)
        ...
        c = p.createConstraint(self.id,
                               ...
                               childFramePosition=[0, 0, 0],physicsClientId = self.physicsClient)
        p.changeConstraint(c, gearRatio=-1, erp=0.1, maxForce=50,physicsClientId = self.physicsClient)

    def move_gripper(self, open_length):
        assert self.gripper_range[0] <= open_length <= self.gripper_range[1]
        for i in [9, 10]:
            p.setJointMotorControl2(self.id, i, p.POSITION_CONTROL, open_length, force=20,physicsClientId = self.physicsClient)
...
class UR5Robotiq85(RobotBase):
    def __init_robot__(self):
        ...
        self.id = p.loadURDF(self.curr_path + '/urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES,physicsClientId = self.physicsClient)
    ...
    def __setup_mimic_joints__(self, mimic_parent_name, mimic_children_names):
        ...
        for joint_id, multiplier in self.mimic_child_multiplier.items():
            c = p.createConstraint(self.id, self.mimic_parent_id,
                                   ...
                                   parentFramePosition=[0, 0, 0],
                                   childFramePosition=[0, 0, 0],physicsClientId = self.physicsClient)
            p.changeConstraint(c, gearRatio=-multiplier, maxForce=100, erp=1,physicsClientId = self.physicsClient)  # Note: the mysterious `erp` is of EXTREME importance
...
class UR5Robotiq140(UR5Robotiq85):
    def __init_robot__(self):
        ...
        self.arm_rest_poses = [-1.5690622952052096, -1.5446774605904932, 1.343946009733127, -1.3708613585093699,
                               -1.5707970583733368, 0.0009377758247187636]
        self.id = p.loadURDF(self.curr_path + '/urdf/ur5_robotiq_140.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES,physicsClientId = self.physicsClient)
        self.gripper_range = [-0.1, 0.025]
